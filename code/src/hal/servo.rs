...
impl ServoDriverMaker {
    ...
    pub fn make_channel(
        ...
        ServoDriver::new(channel, self.frequency, inverted, offset_deg)
    }
}
...
impl ServoDriver {
    ...
    pub fn update(&mut self, angle: i16) {
        let angle = angle + self.offset_deg;
        let mid = ((self.duty_max + self.duty_min) / 2) as i32;
        let range = (self.duty_max as i32) - mid;
        let invert_coeff = if self.inverted { -1 } else { 1 };
        let delta = (range * (angle as i32) / 90) * invert_coeff;
