...
pub async fn do_pid_iteration(
    ...
    let yaw_error = pid_inputs.target_yaw - pid_inputs.motion_data.gyro_yaw;
    let pitch_error = pid_inputs.target_pitch - pid_inputs.motion_data.gyro_pitch;
    let roll_error = pid_inputs.target_roll - pid_inputs.motion_data.gyro_roll;

    let motor_thrust = inputs.throttle() as u16;
    ...
    if !inputs.armed() || motor_thrust < 200 {
        context.reset();
