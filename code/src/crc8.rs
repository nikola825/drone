...
fn crc8_process_byte(current: u8, byte: u8) -> u8 {
    let index = (current ^ byte) as usize;

    CRC8_LOOKUP_0XD5[index]
}

pub fn crc8_calculate(buf: &[u8]) -> u8 {
    let mut crc = 0u8;
    for b in buf {
