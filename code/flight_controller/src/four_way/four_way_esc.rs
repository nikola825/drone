...
pub async fn four_way_loop(
    usb: &mut UsbSerialWrapper,
    params: &mut FourWayParameters,
) -> Result<(), Disconnected> {
    unsafe {
...
async fn handle_4way_passthrough_message(
    ...
    let result = if let Ok(command) = received_command {
        execute_command(params, usb, &command, context).await
...
async fn execute_command(
    ...
    match command.header.command {
        FourWayCommand::cmd_DeviceInitFlash => {
            ...
            let response = device_init_flash(context.pick_esc(params)?).await?;
            send_response(usb, response, &command.header).await?;
            params.leds.yellow_on();
        }
        FourWayCommand::cmd_DeviceRead => {
            let len = command.payload[0];
            let (len, payload) = device_read(
                context.pick_esc(params)?,
        ...
        FourWayCommand::cmd_DeviceReset => {
            let response = device_reset(context.pick_esc(params)?).await?;
            send_response(usb, response, &command.header).await?;
        }
