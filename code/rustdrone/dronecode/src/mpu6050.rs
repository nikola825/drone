...
use embassy_stm32::{
    i2c::{
        Error, ErrorInterruptHandler, EventInterruptHandler, I2c, Instance, RxDma, SclPin, SdaPin,
        TxDma,
    },
    interrupt,
    mode::{Async, Blocking},
...
impl MPU6050 {
    pub fn new<T: Instance>(
        ...
        let i2c: I2c<Async> = I2c::new(
            ...
            tx_dma,
            rx_dma,
            Hertz(mpu_constants::I2C_FREQUENCY),
            Default::default(),
        );
    ...
    pub fn test_connection(&mut self) -> bool {
        ...
        match self.i2c.blocking_write_read(
            mpu_constants::MPU_ADDRESS,
            &[mpu_registers::WHO_AM_I],
            &mut data,
        ) {
            Ok(()) => {
    ...
    async fn write_reg_byte(&mut self, reg_addr: u8, reg_data: u8) {
        self.i2c
            .write(mpu_constants::MPU_ADDRESS, &[reg_addr, reg_data])
            .await
            .unwrap();
    }
    ...
    async fn read_reg_bytes(&mut self, reg_addr: u8, reg_data: &mut [u8]) {
        self.i2c
            .write_read(mpu_constants::MPU_ADDRESS, &[reg_addr], reg_data)
            .await
            .unwrap();
    }
    ...
    pub async fn init(&mut self, gyro_range: GyroRange) {
        self.write_reg_byte(
            mpu_registers::PWR_MGMT_1,
            mpu_constants::PWR_MGMT_1_CKLSEL_X_GYRO_PLL,
        )
        .await;
        self.write_reg_byte(mpu_registers::GYRO_CONFIG, gyro_range as u8)
            .await;
        self.write_reg_byte(
            mpu_registers::ACCEL_CONFIG,
            mpu_constants::ACCEL_CONFIG_AFS_SEL_2,
        )
        .await;
    ...
    pub async fn get_ypr_deg(&mut self) -> (f32, f32, f32) {
        let mut data = [0u8; 6];
        self.read_reg_bytes(mpu_registers::GYRO_XOUT_H, &mut data)
            .await;
